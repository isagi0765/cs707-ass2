...
def parse_args():
    ...
    # dataset
    parser.add_argument('--dataset', default='road_segmentation', help='dataset name')      
    parser.add_argument('--data_dir', default='data_road', help='dataset dir')
...
def main():
    ...
    # Get absolute paths to current script files
    current_dir = os.path.dirname(os.path.abspath(__file__))
    shutil.copy2(os.path.join(current_dir, 'train.py'), f'{output_dir}/{exp_name}/')
    shutil.copy2(os.path.join(current_dir, 'archs.py'), f'{output_dir}/{exp_name}/')
    ...
    def load_split(file_path):
        """Load image IDs from train.txt/val.txt."""
        with open(file_path, 'r') as f:
            return [line.strip() for line in f if line.strip()]

    # Load splits
    train_ids = load_split('/content/drive/MyDrive/Copy of train.txt')  # Use the provided train.txt file
    val_ids = load_split('/content/drive/MyDrive/Copy of val.txt')      # Use the provided val.txt file

    # Dataset paths
    train_img_dir = os.path.join(config['data_dir'], 'training', 'image_2')
    train_mask_dir = os.path.join(config['data_dir'], 'training', 'gt_image_2')
    ...
    train_dataset = RoadSegmentationDataset(
        ...
        img_ids=train_ids,  # Use train IDs from train.txt
    ...
    val_dataset = RoadSegmentationDataset(
        img_dir=train_img_dir,
        mask_dir=train_mask_dir,
        img_ids=val_ids,  # Use validation IDs from val.txt
