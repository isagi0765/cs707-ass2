...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=256, patch_size=16, in_chans=3, 
                 embed_dims=[256, 320, 512], no_kan=False, drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        super().__init__()
        ...
        self.block3 = nn.ModuleList([KANBlock(
            dim=embed_dims[2],
            drop=drop_rate, 
            drop_path=dpr[2], 
            norm_layer=norm_layer
    ...
    def forward(self, x):
        ...
        # Decoder path with KAN blocks
        for blk in self.block1:
            x3 = blk(x3, *x3.shape[2:])
        ...
        return x3
__all__ = [
    'KANLayer', 'KANBlock', 'DWConv', 
    'DW_bn_relu', 'PatchEmbed', 'ConvLayer',
    'D_ConvLayer', 'UKAN'
]
