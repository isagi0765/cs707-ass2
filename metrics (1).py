...
def indicators(output, target):
    ...
    if not np.any(target_):
        return 0, 0, 0, 0, 0, 0, 0  # or np.nan for all metrics

    try:
        iou_ = jc(output_, target_)
        dice_ = dc(output_, target_)
        hd_ = hd(output_, target_) if np.any(output_) else 0
        hd95_ = hd95(output_, target_) if np.any(output_) else 0
        ...
        if "does not contain any binary object" in str(e):
            return 0, 0, 0, 0, 0, 0, 0  # Handle empty predictions
        raise
